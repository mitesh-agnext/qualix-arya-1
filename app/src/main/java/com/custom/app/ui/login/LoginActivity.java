...
import static com.custom.app.util.Constants.OTP_DIALOG;
...
public class LoginActivity extends BaseActivity implements LoginView, OtpDialog.Callback, ValidationListener {
...
//      validator.registerAnnotation(Username.class);
    ...
    @Override
    public void showOtpDialog(LoginResponse response) {
        OtpDialog.newInstance(response.getKey()).show(getSupportFragmentManager(), OTP_DIALOG);
    }
    ...
    public void showHomeScreen(LoginResponse response) {
        ...
        if (!userManager.isLoggedIn()) {
            ...
            userManager.setToken(response.getToken());

            Constants.TOKEN = String.format("Bearer %s", response.getToken());

            userManager.setUsername(user.getUsername());
            userManager.setProfilePath(user.getProfile());
            userManager.setCustomerId(user.getCustomerId());
            if (user.getRoles() != null && !user.getRoles().isEmpty()) {
                if (user.getRoles().contains("admin")) {
                    userManager.setCustomerType("SERVICE_PROVIDER");
                } else if (user.getRoles().contains("customer_admin")) {
            ...
            analytics.setUserProperty("user_id", user.getId());
            analytics.setUserProperty("version", getString(R.string.app_version));

            Bundle bundle = new Bundle();
            bundle.putString(Constants.FLOW, Constants.NAV_LOGIN);
            Utils.Companion.startActivityWithLoad(this, HomeActivity.class, bundle, true);
           // ActivityUtil.startActivity(this, QualityAnalysisActivity.class, false);
        } else {
            finish();
        }
    }
