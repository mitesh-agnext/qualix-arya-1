...
class InstallationCenterListActivity : BaseActivity(), View.OnClickListener, CenterListCallback {
    ...
    private lateinit var viewModel: InstallationCenterListViewModel
    private lateinit var adapterDevice: InstallationCenterListAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this,
                CenterListViewModelFactory(installationCenterInteractor, customerInteractor))[InstallationCenterListViewModel::class.java]
    ...
    private fun initView() {
        toolbar.title = getString(R.string.installation_center)
    ...
    private fun updateUI(screenStateDevice: ScreenState<InstallationCenterListState>?) {
    ...
    private fun processLoginState(renderStateDevice: InstallationCenterListState) {
        progressListDevices.visibility = View.GONE

        when (renderStateDevice) {
            InstallationCenterListState.ListCenterSuccess -> {
                updateRecycleView()
            }
            InstallationCenterListState.ListCenterFailure -> {
            ...
            InstallationCenterListState.GetCustomerListFailure -> {
                AlertUtil.showSnackBar(
                        listLayout,
                        viewModel.errorMassage,
                        2000
            ...
            InstallationCenterListState.GetCustomerListSuccess -> {
                customerList = viewModel.customerList.value!!
                updateCustomerSpinner(customerList)
            }
            InstallationCenterListState.GetDevicesHasConnectionFailure -> {
                AlertUtil.showSnackBar(
                        listLayout,
                        viewModel.errorMassage,
                        2000
            ...
            InstallationCenterListState.CenterDeleteFailure -> {
                AlertUtil.showSnackBar(
                        listLayout,
                        viewModel.errorMassage,
                        2000
            ...
            InstallationCenterListState.CenterDeleteSuccess -> {
                viewModel.centerList.value!!.removeAt(viewModel.deletedPos.value!!)
                viewModel.onGetCenterList(newTextQuery!!, selectedCustomerId!!)
            }
            InstallationCenterListState.GetDevicesTokenExpired -> {
                AlertUtil.showSnackBar(
                        listLayout,
                        "Token expired",
                        2000
    ...
    private fun updateRecycleView() {
        ...
        adapterDevice = InstallationCenterListAdapter(this, viewModel.centerList.value!!, this)
    ...
    override fun onClick(view: View?) {
        when (view) {
            fbAdd -> {
                ActivityUtil.startActivityResult(this, CreateIntlCenters::class.java, Constants.REQUEST_ADD_COLDSTORE, false)
            }
        }
    }
    ...
    override fun editCenterCallback(pos: Int) {
        val intent = Intent(this, UpdateIntlCenters::class.java)
        val gson = Gson()
